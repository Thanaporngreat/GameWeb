...
df = pd.DataFrame({'ActionROG':['New World','Endwalke','Toram Online','The Witcher 3 Wild Hunt','Atomic Heart'],'FPS':['PlayerUnknown Battlegrounds',
'Apex Legends','Warface','Call of Duty','Atomic Heart',],'Racing':['Dirt Rally 2.0','Grid 2','Assetto Corsa','Forza Horizon 4','The Crew 2'],
'Sporgame':['FIFA 22',' CAPTAIN TSUBASA : RISE OF NEW CHAMPIONS','Tony Hawks" Pro sjater','AO Tennis 2',
'eFootball 2022'],'Dungeonrawling':['Diablo Series','Path Of Exile','The Binding Of Isaac','Darkest Dungeon','Enter The Gungeon']})


#from streamlit_extras.let_it_rain import rainF
...
with tab2:
   ...
   new_option = st.text_input('Add new typegame')

with tab3:
    pow = st.selectbox('Choose Game', ['FPS', 'rox'])
    if pow == 'FPS':
        volume = st.columns(2)
        target_weight = volume[0].number_input('น้ำหนักของคุณ', min_value=1)
        colum = st.columns(3)
